...
#include "ui_actions_right.h"

Actions_right::Actions_right(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Actions_right)
{
    ui->setupUi(this);
...
void Actions_right::Type_Action(int type)
{
    switch (type) {
...
void Actions_right::on_pushButton_delAct_clicked()
{
...
void Actions_right::mainWinP(QMainWindow *mainWin)
{
    this->mainWin = mainWin;
    //connect(this, SIGNAL(delAct(int)), this->mainWin, SLOT(delAct_clicked(int)));
}

void Actions_right::actLeftPos(int pos)
{
    this->pos = pos;
}

void Actions_right::setPosNumber(int number)
{
    this->pos = number;
}

void Actions_right::setTextMessege(QString txt)
{    
...
void Actions_right::setTitleDescr(QString title)
{
    ui->label_descr_title->setText("<html><head/><body><p><span style=' font-size:12pt; font-weight:600;'>" + title + "</span></p></body></html>");
}

void Actions_right::setDecriptionDescr(QString descr)
{
...
void Actions_right::setTickedID (int id)
{
    ui->label_descr_id->setText("Ticket #" + QVariant(id).toString() + " description");
}

void Actions_right::setUserDate(QString date)
{
    ui->label_curDate->setText(date);
}

void Actions_right::setUserName(QString usrName)
{
    ui->label_cre_name->setText(usrName);
}

void Actions_right::setCheckWork(int check)
{
    ui->checkBox_work->setChecked(check);
}

void Actions_right::setTitleWork(QString title)
{
    ui->label_work_title->setText(title);
}

void Actions_right::setTextWork(QString txt)
{
...
void Actions_right::setDateRangeWork(QString date)
{
    ui->label_work_dates->setText(date);
}

void Actions_right::setNameWorkerWork(QString name)
{
    ui->label_work_worker->setText(name);
}

void Actions_right::setTextAgree(QString txt)
{
...
void Actions_right::setDate_CurUsrName(QString date, QString name, int agree)
{
    QString agreement;
    if(agree == 3 || agree == 4){
